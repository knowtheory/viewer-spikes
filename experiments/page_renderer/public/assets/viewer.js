...
/*
  When do notes get rendered/loaded?

  when a new note is created
  when a note is edited
  when a page is loaded
  
  Note view should manage user interaction on itself
  Note view should track changes to its model
...
DC.view.PageOverlay = DC.Backbone.View.extend({
  ...
  initialize: function(options) {
    ...
    this.listenTo(this.page, 'loaded', this.load);
    this.listenTo(this.page, 'unloaded', this.unload);
  }
});
...
DC.view.NoteOverlay = DC.view.PageOverlay.extend({
  ...
  initialize: function(options) {
    // javascript "inheritance" my foot. (see: https://github.com/jashkenas/backbone/pull/787 )
    DC.view.PageOverlay.prototype.initialize.call(this, options);
    this.collection = this.page.model.notes;
  },
